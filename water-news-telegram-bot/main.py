...
# gpt-5 계열은 temperature 커스텀 값을 지원하지 않으므로 전송하지 않습니다.
...
def replace_company_names_with_english(text):
    if not text:
        return ""
    out = str(text)
    # 회사명 + 지명 사전을 통합하고, 긴 키부터 치환해 부분 문자열 충돌 방지
...
중요:
- 기업명, 해외 지명, 해외 기관명, 브랜드명, 제품명 등 영어가 원문인 고유명사는 한국어로 음역하지 말고 반드시 영문 원문 그대로 표기하세요. 예: Toray(토레이/도레이 금지), Veolia, SUEZ, Buffalo(버팔로 금지), Tokyo(도쿄 금지), Henderson, Boise. 이 규칙은 ko_title, brief, summary, why_important 모든 필드에 동일하게 적용됩니다.
